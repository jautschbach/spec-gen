...
def gauss(sigma, x, x0):
    return (1.0 / (sigma * sqrt(2.0 * pi))) * exp(
...
try:
    ...
    if arguments["--gaussian"]:
        try:
            ...
            exit(
                "--> Error: {}\nHow to Fix: Gaussian sigma value should be a floating point number.".format(
                    err
                )
            )
    ...
    if arguments["--lorentzian"]:
        try:
            ...
            exit(
                "--> Error: {}\nHow to Fix: Lorentzian gamma value should be a floating point number.".format(
                    err
                )
            )
    ...
    if arguments["--quadrature"]:
        try:
            ...
            exit(
                "--> Error: {}\nHow to Fix: Quadrature value should be an integer number.".format(
                    err
                )
            )
    else:
    ...
    if arguments["--padding"]:
        try:
            ...
            exit(
                "--> Error: {}\nHow to Fix: Padding value should be an integer number.".format(
                    err
                )
            )
    else:
    ...
    if type_spec in ["abs", "ir"]:
        if arguments["--gaussian"]:
            ...
            for i in arange(energy_range[0], energy_range[1] + delta_e, delta_e):
                ...
                for j in range(num_exc):
                    for_int = (
                        impulses[j] * lorentz(broaden, i, energies[j]) / abs_int(i)
                    )
                    imp_val += for_int
                    integral += for_int
                print("{} {}".format(i, imp_val))
    ...
    with open("spec-gen.log", "w") as f:
        ...
        f.write(
            "Number of points in broadened spectrum: {}\nEnergy padding factor: {}\n".format(
                num_points, factor
            )
        )
        # Integration
        ...
        f.write(
            "Integral of broadened spectrum: {} (Should be 1.00)\n".format(integral)
        )
